...
import com.suncar.suncartrabajador.data.services.MantenimientoApiService
...
class MantenimientoService {
    private val mantenimientoApiService: MantenimientoApiService = RetrofitClient.createService()
    ...
    /**
     * Guarda localmente el mantenimiento en un archivo JSON (lista de mantenimientos no enviados)
     */
    fun guardarMantenimientoLocal(context: Context, mantenimientoRequest: MantenimientoRequest) {
        val gson = Gson()
        val mantenimientos = leerMantenimientosLocales(context)
        mantenimientos.add(mantenimientoRequest)
        val json = gson.toJson(mantenimientos)
        val file = File(context.filesDir, ARCHIVO_MANTENIMIENTOS_PENDIENTES)
        file.writeText(json)
    }

    /**
     * Lee la lista de mantenimientos locales (no enviados) desde el archivo JSON
     */
    private fun leerMantenimientosLocales(context: Context): MutableList<MantenimientoRequest> {
        val file = File(context.filesDir, ARCHIVO_MANTENIMIENTOS_PENDIENTES)
        if (!file.exists()) return mutableListOf()
        val json = file.readText()
        return try {
            val gson = Gson()
            val type = object : TypeToken<MutableList<MantenimientoRequest>>() {}.type
    ...
    /**
     * Envía el formulario de mantenimiento al servidor usando multipart/form-data
     */
    suspend fun enviarMantenimientoMultipart(
        ...
        try {
            return mantenimientoApiService.enviarMantenimiento(
                tipoReporte,
                brigada,
                materiales,
                cliente,
    ...
    /**
     * Envía un MantenimientoRequest local (con imágenes en base64) al backend usando multipart/form-data
     */
    suspend fun enviarMantenimientoLocalMultipart(request: MantenimientoRequest): Result<MantenimientoResponse> {
        return try {
            ...
            val descripcionBody = request.descripcion.toRequestBody("text/plain".toMediaTypeOrNull())
            ...
            val response = enviarMantenimientoMultipart(
                tipoReporteBody,
                brigadaBody,
                materialesBody,
                clienteBody,
    ...
    companion object {
        private const val ARCHIVO_MANTENIMIENTOS_PENDIENTES = "mantenimientos_pendientes.json"
    }
} 